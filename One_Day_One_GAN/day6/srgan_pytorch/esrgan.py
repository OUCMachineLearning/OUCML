...
parser.add_argument("--dataset_name", type=str, default="dtd", help="name of the dataset")
...
parser.add_argument("-c","--checkpoint_interval", type=int, default=5000, help="batch interval between model checkpoints")
parser.add_argument("--residual_blocks", type=int, default=9, help="number of residual blocks in the generator")
parser.add_argument("-w","--warmup_batches", type=int, default=5000, help="number of batches with pixel-wise loss only")
parser.add_argument("--lambda_adv", type=float, default=5e-1, help="adversarial loss weight")
...
dataloader = DataLoader(
    ImageDataset("../data/%s/images/*/" % opt.dataset_name, hr_shape=hr_shape),
...
# discriminator.load_state_dict(torch.load(f"saved_models/discriminator_{0}.pth"))
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, imgs in enumerate(dataloader):
        ...
        if batches_done % 10 == 0:
            print(
                "[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f, content: %f, adv: %f, pixel: %f][ETS: %s]"
                % (
                    epoch,
                    opt.n_epochs,
                    i,
